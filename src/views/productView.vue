    ...
    export default {
        ...
        computed: {
            product() {
                ...
                console.log('Product ID', productId) //ID is being rendered??? :(
                return productStore.getProductById(productId) //Maybe this returns an array of Id's? Should give error?
            },
        },
    }
</script>
<template>
    <section class="product__details"> <!-- Renders this section and the div below but not the image or the block -> (It cannot find the 'product') -->
        <div class="single__product__image">
            <img v-if="product" v-bind:src="'src/assets/' + product.image" :alt="product.title" id="main__image" width="100%">
        </div>
        ...
        <div v-if="product" class="single__product__details">
            ...
            <select>
                ...
                <option v-for="size in product.sizes" :key="size">{{ product.size }}</option>
            </select>
